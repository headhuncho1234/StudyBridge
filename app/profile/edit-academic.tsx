  ...
  useEffect(() => {
    ...
    supabase.auth.getUser().then(async ({ data: userData }) => {
      ...
      const { data } = await supabase
        .from('profiles')
        .select('degree_level, field_of_study')
        .eq('user_id', user.id)
        .maybeSingle();
          ...
          <View style={styles.flexFill}>
            ...
            {loading ? (
              ...
              <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
                ...
                <Text style={styles.heading}>Edit Academic Info</Text>
                ...
                <View style={styles.field}>
                  ...
                  <View style={styles.chipRow}>
                    {DEGREE_LEVELS.map((level) => (
                      <TouchableOpacity
                        key={level.value}
                        style={[styles.chip, degreeLevel === level.value && styles.chipActive]}
                        onPress={() => setDegreeLevel(level.value)}
                        activeOpacity={0.8}
                      >
                        <Text style={[styles.chipText, degreeLevel === level.value && styles.chipTextActive]}>
                          {level.label}
                        </Text>
                      </TouchableOpacity>
                    ))}
